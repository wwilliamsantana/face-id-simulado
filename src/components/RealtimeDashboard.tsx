...
export const RealtimeDashboard = ({ activeStudents, totalStudents }: RealtimeDashboardProps) => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      {/* Class Information */}
      <Card className="lg:col-span-2 bg-zinc-200 border-white/20 backdrop-blur-sm">
        <CardHeader>
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <p className="text-lg font-semibold text-black/80">{mockClassInfo.subject}</p>
            </div>
            <div className="space-y-2">
              <p className="text-sm font-medium text-black/80/70">Professor</p>
              <p className="text-lg font-semibold text-black/80">{mockClassInfo.professor}</p>
            </div>
            <div className="space-y-2">
              <p className="text-sm font-medium text-black/80/70">Local</p>
              <div className="flex items-center space-x-2">
                <MapPin className="w-4 h-4 text-black/80/70" />
                <p className="text-lg font-semibold text-black/80">{mockClassInfo.room}</p>
              </div>
            </div>
            <div className="space-y-2">
              ...
              <div className="flex items-center space-x-2">
                ...
                <p className="text-lg font-semibold text-black/80">{mockClassInfo.duration}</p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-sm font-medium text-black/80/70">Progresso da Aula</p>
              <Badge variant="outline" className="border-white/20 text-black/80">
                {classStatus}
              </Badge>
            </div>
            <Progress value={classProgress} className="h-3" />
            <p className="text-sm text-black/80/70">{Math.round(classProgress)}% concluído</p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 pt-4 border-t border-white/10">
            ...
            <div className="text-center">
              <p className="text-2xl font-bold text-black/80">{totalStudents - activeStudents}</p>
      ...
      <div className="space-y-6">
        ...
        <Card className="bg-zinc-200 border-white/20 backdrop-blur-sm">
          ...
          <CardContent className="space-y-4">
            <div className="text-center">
              <p className="text-3xl font-bold text-black/80">{attendanceRate}%</p>
          ...
          <CardContent className="space-y-3">
            {modalityStats.map((stat) => (
              <div key={stat.type} className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  <span className="text-sm font-medium text-black/80">{stat.count}</span>
                  {stat.type === "Presencial" && <Users className="w-4 h-4 text-black/80/50" />}
                  {stat.type === "Híbrido" && <Monitor className="w-4 h-4 text-black/80/50" />}
                  {stat.type === "EAD" && <Smartphone className="w-4 h-4 text-black/80/50" />}
                </div>
              </div>
            ))}
          </CardContent>
